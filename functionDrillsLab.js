...
let jsNinja = () => {
  let result = "I am a JavaScript ninja!"
  console.log(result)
  return result
}
...
let nameGreeting = nameCheck("Joi")
...
let colorRating = faveColorFinder("teal")
...
function theEliminator(contestants, loser) {
  for(let i = 0; i < contestants.length; i++) {
    if(contestants[i] === loser) {
      removedName = contestants.splice(i, 1)
    } 
  } return contestants
}
...
let myEmail = "j.misenti@berkeley.edu"
emailCheck(myEmail)
...
function chocolateFrog(gold) {
  let frogCost = 3
  let frog = gold / frogCost
  return frog
}

let totalFrogs = chocolateFrog(30)
...
function chocolateFrog2(gold) {
  let frogCost = 3
  let frog = gold / frogCost
  return Math.floor(frog)
}

let totalFrogs2 = chocolateFrog2(4)
